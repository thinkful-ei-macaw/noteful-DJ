...
class App extends React.Component {
  ...
  render() {
    ...
    //   d = list.modified
    //   return (
    //     <Link to={`/note/${list.id}`}><li key={list.id} id={list.id} onClick={e => this.handleNoteClick(list)}>
    //       <h2>{list.name}</h2>
    //       <span>Date modified on </span>
    //       <Moment>{d}</Moment>
    ...
    //   return (
    //     <Link to={`/folder/${list.id}`} onClick={e => this.handleFolderClick(e.target.id)}
    //   ><li key={list.id} id={list.id} >{list.name}</li></Link>
    //   )
    // })
    
    // if (this.state.selectedFolder !== null) {
    //   const newList = this.state.notes.filter((note) => note.folderId === this.state.selectedFolder);
    //   const theNote = newList.map(note => note)
    //   console.log(theNote)
    //   noteList = newList.map(list => {
    //     d = list.modified
    //     return (
    //       <Link to={`/note/${list.id}`}><li key={list.id} id={list.id} onClick={e => this.handleNoteClick(theNote)}>
    //         <h2>{list.name}</h2>
    //         <span>Date modified on </span>
    //         <Moment>{d}</Moment>
    //         <button type="button">Delete Note</button>
    ...
    //   const theSelectedNote = this.state.notes.filter((note) => note.id === this.state.selectedNoteId)
    //   noteList = theSelectedNote.map((list) => {
    //     return (
  ...
  return (
    ...
    <Switch>
      ...
      <Route
        exact
        path="/"
        render={(routeProps) => <Folders {...routeProps} folders={this.state.folders}/>}
      />
      <Route
        exact
        path="/"
        render={(routeProps) => <Notes {...routeProps} notes={this.state.notes}/>}
      />
      <Route
      exact
      path={`/folder/:folderId`}
      render={(routeProps) => {
      let folderList = this.state.folders.filter((folder) => folder.id === routeProps.match.params.folderId);
      let noteList = this.state.notes.filter((note) => note.folderId === routeProps.match.params.folderId);
      // eslint-disable-next-line no-unused-expressions
      <SelectedFolder folderList={folderList} noteList={noteList} {...routeProps} />
    }
  }
      />
      <Route
        exact
        path={`/note/:${this.state.selectedNoteId}`}
        render={() => <SelectedNote selected={this.state.selectedNoteId} note={this.state.selectedNote} content={this.state.selectedNote.content}/>}
        />
    </Switch>
    </div>
  )
